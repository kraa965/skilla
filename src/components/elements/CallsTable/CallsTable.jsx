...
function CallsTable({ data, loading }) {
  ...
  const call_btn = (status) => {
    ...
    if (status === '105') {
      return <button className="outline outline-2 px-2 outline-blue-400 rounded bg-blue-100">Хорошо</button>;
    }
    if (status === '103') {
      return <button className="outline outline-2 px-2 outline-red-400 rounded text-red-400 bg-red-100">Плохо</button>;
    }
  };
    ...
    <div className="container mx-auto">
      {loading ? (
        ...
        <table className="min-w-full bg-white rounded">
          <thead>
            <tr>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Тип</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Время</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Сотрудник</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Звонок</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Источник</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Оценка</th>
              <th className="py-3 px-4 text-gray-600 font-medium border-b">Длительность</th>
            </tr>
          </thead>
          <tbody>
            {data.map((call, index) => (
